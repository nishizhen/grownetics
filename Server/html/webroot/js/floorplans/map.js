...
(function() {
    ...
    GrowServer.Map = function( options) {
        ...
        var DEFAULT_DATA_OVERLAY = "<div class=\"wi wi-thermometer\">Temperature High</div>";
        ...
        this.updateDataPoint = function(dataPoint) {
            ...
            if (mapIcon) {
                var data_time = moment.utc(dataPoint.created, 'YYYY-MM-DD[T]HH:mm[Z]');
                var current_time = moment.utc(moment().subtract(120, 'seconds'), 'YYYY-MM-DD[T]HH:mm[Z]');
                // display data < 2 minutes old
                if (data_time.isAfter(current_time)) {
                    var oldValue = mapIcon.html();
                    var newValue = dataPoint.value;
                    if (GrowServer.showMetric == false && (dataPoint.type == 1 || dataPoint.type == 3)) {
                        newValue = parseFloat((dataPoint.value * 9/5) + 32).toFixed(1);
                    } else {
                        newValue = dataPoint.value;
                    }
                    if (parseFloat(oldValue) != parseFloat(newValue)) {
                        if (parseFloat(newValue) < parseFloat(oldValue)) {
                            $(mapIcon).offsetParent().find('.pulse').addClass('pulse_rays_down').one('animationend webkitAnimationEnd oAnimationEnd MSAnimationEnd', function(){$(this).removeClass('pulse_rays_down')});
                        } else {
                            $(mapIcon).offsetParent().find('.pulse').addClass('pulse_rays_up').one('animationend webkitAnimationEnd oAnimationEnd MSAnimationEnd', function(){$(this).removeClass('pulse_rays_up')});
                        }
                        mapIcon.parent().css('background-color', GrowServer.Color.getColorForSensorTypeAndValue(mapIcon.parent().data('sensortype'), dataPoint.value));
...
// leafletMap.addLayer(mapboxTiles);
        ...
        GrowServer.Map.loading.update = function (props) {
            this._div.innerHTML = '<i class="fa fa-circle-o-notch fa-spin fa-3x fa-fw"></i><span class="sr-only">Loading...</span>';
        };
        ...
        this.leafletMap.on('baselayerchange', function(e) {
            switch(e.name) {
                case '<div class="wi wi-humidity">Humidity Low</div>':
                case '<div class="wi wi-humidity">Humidity High</div>':
                    GrowServer.setMapDataType(2);
                    break;
                case '<div class="wi wi-thermometer">Temperature High</div>':
                case '<div class="wi wi-thermometer">Temperature Low</div>':
                    GrowServer.setMapDataType(3);
                    break;
                case '<div class="fa fa-percent">CO2</div>':
                    GrowServer.setMapDataType(4);
                    break;
                case '<div class="wi wi-lightning">PAR Sensor</div>':
                    GrowServer.setMapDataType(11);
                    break;
                case '<div class="wi wi-raindrop">PH</div>':
                    GrowServer.setMapDataType(5);
                    break;
                case '<div class="wi wi-dust">EC</div>':
                    GrowServer.setMapDataType(7);
                    break;
                case '<div class="wi wi-humidity">DO</div>':
                    GrowServer.setMapDataType(6);
                    break;
                case '<div class="wi wi-raindrops">Waterproof Temperature</div>':
        ...
        GrowServer.Map.getDataLayer = function getDataLayer(dataType) {
            ...
            switch (dataType) {
                ...
                case "Temperature Low":
                    label = '<div class="wi wi-thermometer">'+dataType + '</div>';
                ...
                case "Co2":
                    ...
                    break;
                case "CT":
                    label = '<div class="wi wi-lightning">' +dataType + '</div>';
                    sensorType = "CT";
                    break;
                case "PAR":
                    label = '<div class="wi wi-lightning">' +dataType + '</div>';
                ...
                case "Waterproof Temperature":
                    label = '<div class="wi wi-raindrops">' +dataType+ '</div>';
                    ...
                    legend.onAdd = function (map) {
                        ...
                        switch(sensorType) {
                            ...
                            case "Air Temperature":
                                ...
                                break;
                            case "Co2":
                                grades = [400, 500, 600, 700, 800, 900, 1000, 1100, 1200];
                                break;
                            case "CT":
                        ...
                        if (grades) {
                            ...
                            for (var i = 0; i < grades.length; i++) {
                                if (GrowServer.showMetric == false && sensorType == 'Air Temperature'){
                                    var color = GrowServer.Color.getColorForSensorTypeAndValue(sensorType, ((grades[i]+1) - 32) * 5/9);
                                } else {
                                    var color = GrowServer.Color.getColorForSensorTypeAndValue(sensorType, grades[i]+1);
                                }
                                div.innerHTML +=
        ...
        GrowServer.Map.getMarkerForMapItemType = function getMarkerForMapItemType(mapItem, mapItemType, options) {
            var icon;
            ...
            switch(mapItemType) {
                case "Sensor":
                    return L.marker([mapItem.latitude, mapItem.longitude], {
                        icon:  L.divIcon({
                            ...
                            html: '<div class="pulse_holder"><div class="pulse_marker"><div class="pulse"></div></div></div><div id="sensor-value" data-sensorType="'+options.sensor_type+'" style="background-color:' + options.color + ';"><span id="grownetics-sensor-'+options.id+'">'+(options.value ? options.value : '?') +'</span><span id="sensor-symbol">'+symbol+'</span></div>'
                        })
                    }).bindPopup(
                        "<h6>"+mapItem.label+"</h6>"
                    );
                ...
                case "Plant Placeholder":
                    return L.circleMarker([mapItem.latitude, mapItem.longitude], {
                        radius: 4,
                        color:  GrowServer.Color.getColorForMapItemType("Plant Placeholder"),
                        weight: 1,
                        opacity: 0.66,
                        fillOpacity: 0.33
                ...
                default:
                    ...
                    if (!icon && mapItem.geoJSON) {
                        ...
                        marker = L.marker([mapItem.latitude,mapItem.longitude], {
                            title: mapItem.label,
                            icon: icon
                        });
                    }
        ...
        this.createPlantsLayer = function (data) {
            ...
            for (var kk = 0; kk < plants.length; kk++) {
                ...
                L.circleMarker([plants[kk].map_item.latitude, plants[kk].map_item.longitude], plantMarker
                ).bindPopup(
                    '<div style="font-size:1.25em;">' +
                    "<p>Plant ID: " + plants[kk].plant_id + "</p>" +
                    "<p>Zone: <a href='/zones/view/" + plants[kk].zone.id + "'> " + plants[kk].zone.label + "</a></p>" +
                    "<p>Batch: <a href='/harvestBatches/view/" + plants[kk].harvest_batch_id + "'>" + plants[kk].harvest_batch.cultivar.label + " - Batch #" + plants[kk].harvest_batch.batch_number + "</a></p>" +
                    "<p>Status: <b>" + statusToValue(plants[kk].status) + "</b></p>" +
                    "</div>"
                ).openPopup().addTo(plantGroup);
            }
        ...
        this.createSensorsLayer = function (data) {
            ...
            for (var i = 0; i < sensors.length; i++) {
                ...
                if (sensorType == "Humidity") {
                    sensorMarker = GrowServer.Map.getMarkerForMapItemType(sensor.map_item, "Sensor", {
                        "id": sensor.id,
                        "color": color,
                        "symbol": symbol,
                    ...
                    if (height == 1) {
                        sensorMarker.addTo(GrowServer.Map.getDataLayer("Humidity Low"));
                    } else if (height == 2) {
                        sensorMarker.addTo(GrowServer.Map.getDataLayer("Humidity High"));
                    }
                    ...
                    if (height == 1) {
                        sensorMarker.addTo(GrowServer.Map.getDataLayer("Temperature Low"));
                    } else if (height == 2) {
                        sensorMarker.addTo(GrowServer.Map.getDataLayer("Temperature High"));
                    }
                    ...
                    sensorMarker = GrowServer.Map.getMarkerForMapItemType(sensor.map_item, "Sensor", {
                        "id": sensor.id,
                        "color": color,
                        "symbol": "<span style='font-size: 0.5em;'>" + symbol + "</span>",
                        "sensor_type": sensorType
                    });
                    sensorMarker.addTo(GrowServer.Map.getDataLayer(sensorType));
        ...
        this.createPlantPlaceholdersLayer = function(data) {
            ...
            if (plantPlaceholders && plantPlaceholders.length) {
                for (var kk = 0; kk < plantPlaceholders.length; kk++) {
                    ...
                    GrowServer.Map.getMarkerForMapItemType(plantPlaceholder, plantPlaceholder.map_item_type.label).addTo(layers["Plant Placeholders"]);
                }   
            }
        }
        ...
        this.createMapItemsLayer = function(data) {
            ...
            if (mapItems && mapItems.length) { // sometimes maybe mapItems fails?
                for (var ix = 0; ix < mapItems.length; ix++) {
                    ...
                    if ("Zone" == mapItemType) {
                        ...
                        } else if(!layers["Benches"]) {
                            layers["Benches"] = L.featureGroup();
                        } else if(!layers["Rooms"]) {
                            layers["Rooms"] = L.featureGroup();
                        } else if(!layers["Custom_Zones"]) {
                            layers["Custom_Zones"] = L.featureGroup();
                        }
                        if (mapItem.zones) {
                            if (mapItem.zones[0].zone_type_id == 1) {
                                GrowServer.Map.getMarkerForMapItemType(mapItem, mapItemType).addTo(layers["Rooms"]);
                            } else if (mapItem.zones[0].zone_type_id == 2) {
                                GrowServer.Map.getMarkerForMapItemType(mapItem, mapItemType).addTo(layers["HVAC"]);
                            } else if (mapItem.zones[0].zone_type_id == 3) {
                                GrowServer.Map.getMarkerForMapItemType(mapItem, mapItemType).addTo(layers["Benches"]);
                            } else {
...
GrowServer.Map.getMarkerForMapItemType(mapItem, mapItemType, {
                                "style": {
                                    lineCap: "round",
                                    lineJoin: "miter"
                                }
                            ...
                            GrowServer.Map.getMarkerForMapItemType(mapItem, mapItemType).addTo(layers["Room Labels"]);

                        } else { // Add Everything Else to 'Appliances'
                            var marker = GrowServer.Map.getMarkerForMapItemType(mapItem, mapItemType, {
                                "style": {
                                    fill: true,
        ...
        ///// ajax layer init /////
        var floorplanPromise = $.ajax('/floorplans/layers/walls/' + GrowServer.Floorplan.id + '.json').then(this.createFloorplanLayer);
        var sensorsPromise = $.ajax('/floorplans/layers/sensors/' + GrowServer.Floorplan.id + '.json').then(this.createSensorsLayer);
        var plantsPromise = $.ajax('/floorplans/layers/plants/' + GrowServer.Floorplan.id + '.json').then(this.createPlantsLayer);
        var mapItemsPromise = $.ajax('/floorplans/layers/map_items/' + GrowServer.Floorplan.id + '.json').then(this.createMapItemsLayer);
        var plantPlaceholdersPromise = $.ajax('/floorplans/layers/plant_placeholders/' + GrowServer.Floorplan.id + '.json').then(this.createPlantPlaceholdersLayer);

        $.when(floorplanPromise, sensorsPromise, plantsPromise, mapItemsPromise, plantPlaceholdersPromise).done(function() {
            // create layers
            var initialLayers = {};

            for (var layerName in layers) { //; i < layers.length;i++) {
            ...
            control.onAdd = function(map) {
                    ...
                    L.DomEvent
                        ...
                        .addListener(azoom, 'click', function() {
                            GrowServer.Floorplan.leafletMap.setView(GrowServer.Floorplan.center, 15);
                        },azoom);
                    return azoom;
                };
            return control;
