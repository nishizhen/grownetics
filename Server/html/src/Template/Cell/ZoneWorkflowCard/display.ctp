<?php if (sizeof($tasks) > 0): ?>
    ...
    <div class="panel-heading">
        <div class="pull-left"><h4><?=$this->Html->link($zone->label, ['controller' => 'Zones', 'action' => 'view', $zone->id]);?></h4></div>
        <br>
      </div>
        ...
        $task->type == $this->Enum->enumValueToKey('Tasks', 'type', 'Harvest')) {
            ...
            >
              ...
              <div class="task-title">
                <span class="task-title-sp" data-taskID="<?=$task->id?>"><span id="taskLabel"><?php 
                ...
                ?></span><?php } ?> <?php if ($task->status == $this->Enum->enumValueToKey('Tasks', 'status', 'Incomplete')): ?> Assigned to: <span
                        ...
                        ><?=$this->
                        Html->link($task->user->name, ['controller' => 'Users', 'action' => 'view', $task->user->id]);?></span></span></span>
...
by <?=$this->Html->link($task->user->name, ['controller' => 'Users', 'action' => 'view', $task->user->id]);?> </span>
              <?php endif; ?>
                <div class="pull-right hidden-phone">
                    ...
                    <button class="btn btn-success btn-xs completeFacilityTaskBtn"><i class="fa fa-check"></i><i style = "position:relative; display: none;" class="completeTaskSpinner fa fa-circle-o-notch fa-spin fa-1x fa-fw"></i></button>
                  <button class="btn btn-primary btn-xs editFacilityTaskBtn"><i class="fa fa-pencil"></i></button>
