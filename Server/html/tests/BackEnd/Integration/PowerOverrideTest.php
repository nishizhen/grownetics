...
class PowerOverrideTest extends IntegrationTestCase {
    ...
    public function testPowerOnOverride()
    {
        $output = $this->Outputs->get($this->relay_output_id);
        $output->status = $this->Outputs->enumValueToKey('status','Force On');
        $this->Outputs->save($output);
        ...
        // Test high current (should turn off the light) doesn't actually do it due to Force On.
    ...
    public function testPowerOffOverride()
    {
        $output = $this->Outputs->get($this->relay_output_id);
