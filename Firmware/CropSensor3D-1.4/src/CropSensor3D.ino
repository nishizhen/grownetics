...
void readAnalogPin(char *data, int pinNumber)
{
  ...
  for (int i=0; i<20; i++)
  {
      reading = reading + ads.readADC_SingleEnded(pinNumber);
  }
  reading = reading / 20;
  ftoa(buffer, reading, 2); //itoa? readADC_SingleEnded returns an int.
  strcat(data, buffer);
  strcat(data, "]");
}
...
void sendDataToServer()
{
  ...
  if (!client.connected() && (millis() - lastConnectionTime > refreshRate))
  {
    ...
    tcaselect(0); // ADS1015 is on channel 0 of mux
    for (int ADS1015_pin = 0; ADS1015_pin < 4; ADS1015_pin++) { // loop through all pins on ADS1015
    if (strlen(data) != 0)
    {
      strcat(data, ",");
    }
    readAnalogPin(data, ADS1015_pin);
    }
