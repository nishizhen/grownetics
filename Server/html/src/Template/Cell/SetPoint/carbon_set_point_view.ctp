...
<?php if (isset($setPoint)): ?>
	...
	<?php if ($setPoint->default_setpoint_id != 0 && $this->request->params['controller'] == 'Zones'): ?> 
		border:1px solid rgba(105,224,105,.5); color: rgba(0, 0, 0, 0.5); 
	<?php else: ?> 
		border:1px solid rgb(105,224,105); font-weight:bold; 
	<?php endif; ?>" 
		type="number"
	...
	<button 
		...
		class="btn btn-xs fa fa-check btn-success SaveSetPoint" 
		...
		data-data_type="<?=$co2SensorType?>"></button>
	<button 
		style="display:none; font-size: 0.9em;" 
		class="btn btn-xs btn-danger fa fa-times RevertSetPoint">
	</button>
</div>
	<button style="font-size: 0.7em; <?php if ($setPoint->default_setpoint_id != 0): ?>  display:none; <?php endif; ?>" class="btn btn-xs btn-theme ResetToDefault fa fa-refresh"></button>
<?php endif; ?>
