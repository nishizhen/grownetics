...
$(".editFacilityTaskBtn").on('click', function(e) {
	...
	row.find('.completeFacilityTaskBtn').toggle();
	...
	// /task date edit
	row.find('.cancelEditFacilityTaskBtn').show();
	row.find('.submitEditFacilityTaskBtn').show();

	row.find('.cancelEditFacilityTaskBtn').off('click').click(function(e) {
		row.find('.task-title').css('overflow', 'hidden');
		row.find('.cancelEditFacilityTaskBtn').hide();
		row.find('.submitEditFacilityTaskBtn').hide();
		row.find('.editFacilityTaskBtn').show();
		row.find('.completeFacilityTaskBtn').show();
	...
	row.find('.submitEditFacilityTaskBtn').off('click').click(function(e) {
		row.find('.task-title').css('overflow', 'hidden');
		...
		$.ajax({
        	...
        	success: function(data) {
        		...
        		if (new_zone != 0) {
					row.find('.zone').html('<a href=/zones/view/'+new_zone+'>'+row.find(".text").text()+'</a>');        			row.find('.zone').data(new_zone);
        			row.find('.zone').attr('data-value', new_zone);
					row.find('.text').attr('data-value', new_zone);
        			row.find('.zone').data(new_zone);
        		}
                if (new_date != '') {
                   row.find('#taskDate').html(moment(new_date).format('M/DD/YY')); 
                } else {
                    row.find('#taskDate').html(''); 
                }
        		row.find('.editFacilityTaskBtn').show();
				row.find('.completeFacilityTaskBtn').show();
				row.find('.deleteTaskBtn').show();
...
$(".completeFacilityTaskBtn").on('click', function() {
