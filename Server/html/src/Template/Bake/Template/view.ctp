    ...
    foreach ($associations as $type => $data) {
        foreach ($data as $alias => $details) {
            if ($details['controller'] !== $this->name && !in_array($details['controller'], $done)) {
                echo $this->Html->link(__('List '.$this->_pluralHumanName($alias)), ['controller' => $details['controller'], 'action' => 'index']);
                echo $this->Html->link(__('New '.Inflector::humanize(Inflector::singularize(Inflector::underscore($alias)))), ['controller' => $details['controller'], 'action' => 'add']);
