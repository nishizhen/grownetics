...
(function() {
    ...
    GrowServer.Floorplan.Importer = function( options) {
        ...
        if (!this.map) {
            ...
            this.map = new L.Map("floorplan-map", {center: [40.01734, -105.27168], zoom: 4}); // Boulder
        }
        ...
        this.updateProgress = function( callback) {
            ...
            for (var i = 0; i < this.progressHandlers.length; i++) {
                try {
                    this.progressHandlers[i].apply(this, Array.prototype.slice.apply(arguments, [1]));
                } catch( exc) {
    ...
    GrowServer.Floorplan.Importer.prototype = {
        ...
        importFloorplan: function() {
            ...
            if (!floorplanElement) {
                console.log("failed to find floorplan walls element. retrying...");
                floorplanElement = this.getSVGElement(this.svgXml, "G-Floorplan");
            }
            ...
            floorplanWalls.selectAll("line").each(function () {
                ...
                try {
                    var polyline = L.polyline([
                        theMap.containerPointToLatLng(L.point(parseFloat(line.attr("x1")) * self.xConvFactor, parseFloat(line.attr("y1")) * self.yConvFactor), self.zoomScale),
                        theMap.containerPointToLatLng(L.point(parseFloat(line.attr("x2")) * self.xConvFactor, parseFloat(line.attr("y2")) * self.yConvFactor), self.zoomScale),
                    ]);
            ...
            floorplanWalls.selectAll("polyline").each(function () {
                ...
                for (var i = 0; i < points.length; i++) {
                    ...
                    if (coords && coords.length == 2) {
                        mapPoints.push(theMap.containerPointToLatLng(L.point(parseFloat(coords[0]) * self.xConvFactor, parseFloat(coords[1]) * self.yConvFactor), self.zoomScale));
                    }
                }
            ...
            floorplanWalls.selectAll("rect").each(function () {
                ...
                try {
                    var rectangle = L.rectangle([
                        theMap.containerPointToLatLng(L.point(x * self.xConvFactor, y * self.yConvFactor), self.zoomScale),
                        theMap.containerPointToLatLng(L.point((x + parseFloat(rect.attr("width"))) * self.xConvFactor, (y + parseFloat(rect.attr("height"))) * self.yConvFactor), self.zoomScale)
                    ]);
        ...
        importZones: function() {
                ...
                if ( plant_zone_type != "Plant Zone") {
                    d3.select(this).selectAll("rect").each(function () {
                        ...
                        try {
                            var x = parseFloat(rect.attr("x"));
                            var y = parseFloat(rect.attr("y"));
                            var zone_id = rect.attr("id").replace(/_$/, '');
                            ...
                            if ( plant_zone_type != "Room"
                                && plant_zone_type != "HVAC"
                                && plant_zone_type != "Custom" ) {  
                                self.roomZones[zone_id] = rectangle;
                                self.plantZoneTypes[zone_id] = plant_zone_type;
                                zone_type = "Room";
                            }
                            ...
                            self.zones_geoJSON.push(rectangle.toGeoJSON());
                        } catch (exc) {
            ...
            this.updateProgress(this.importPlantZones, 'zones', this.zones.length);

        },

        importPlantZones: function() {
            var self = this;
            var plant_zones = d3.select(this.getSVGElement(this.svgXml, "Plant_Zone"));
            ...
            plant_zones.selectAll("rect").each(function () {
                ...
                try {
                    ...
                    var rectangle = L.rectangle([
                        self.map.containerPointToLatLng(L.point(x * self.xConvFactor, y * self.yConvFactor), self.zoomScale),
                        self.map.containerPointToLatLng(L.point((x + parseFloat(rect.attr("width"))) * self.xConvFactor, (y + parseFloat(rect.attr("height"))) * self.yConvFactor), self.zoomScale)
                    ]);
            ...
            this.updateProgress(this.importDevices, 'plant_zones', this.plant_zones.length);
        },
        ...
        importDevices: function() {
            ...
            if (!deviceElement) {
                ...
                deviceElement = this.getSVGElement(this.svgXml, "_x33_D_Crop_Device");
                if (!deviceElement) {
                    console.log("failed twice to find device element. retrying...");
...
console.log(deviceLabel);
                ...
                // var bounds = sensorGroup.getBounds();
                var center = deviceGroup.getBounds().getCenter();
                //var zone_id = self.zoneForMapItemCenter(center);
            ...
            this.updateProgress(this.importPlants, 'devices', this.devices.length);
        },
        ...
        importPlants: function() {
            ...
            // Plants
            var ppIndex = 0, plantPlaceholdersGroup = L.featureGroup();
            plantPlaceholdersGroup.addTo(theMap);
            d3.select(this.getSVGElement(this.svgXml, "Plant_Placeholders")).selectAll("circle").each(function () {
                ...
                var center = theMap.containerPointToLatLng(L.point(cx * self.xConvFactor, cy * self.yConvFactor), self.zoomScale);
                ...
                if (circle) {
                    ...
                    if (zone_id) {
                        ...
                        self.plant_placeholders[zone_id].push({
                            "zone_id": zone_id,
                            "ordinal": self.plant_placeholders[zone_id].length,
                            "label": self.prettifyLabel(zone_id) + " Plant Placeholder " + (self.plant_placeholders[zone_id].length + 1),
                            "type": "Plant Placeholder",
                            "latitude": center.lat,
                            "longitude": center.lng
                        });
                        self.plant_placeholders_geoJSON[zone_id].push(circle.toGeoJSON());
                        ppIndex++;
                    } else {
                        console.warn("failed to locate zone for plant placeholder: ", circle);
                    }
                }
            });

            this.updateProgress(this.importMapItems, 'plant_placeholders', ppIndex);
        },
        ...
        importMapItems: function() {
            ...
            d3.select(this.getSVGElement(this.svgXml, "Appliances")).selectAll("#Appliances > g").each(function() {
                var groupId = d3.select(this).attr("id");
                if (groupId) {
                    if (groupId == "Lights") {
                        self.processAppliances(self.svgXml, groupId, appliancesGroup);
                    } else {
                        self.processMapItemGroup(self.svgXml, groupId, mapItemsGroup);
                    }
                }
            });
        ...
        processAppliances: function processAppliances(svgXml, groupName, layerGroup) {
            var idx = 0;
            var self = this;
            d3.select(this.getSVGElement(this.svgXml, groupName)).selectAll("#"+groupName + " > g").each(function() {
                ...
                d3.select(this).selectAll("g").each(function() {
                    d3.select(this).selectAll("path").each(function() {
                        ...
                        if (linePath) {
                            ...
                            var zones = self.zonesForMapItem(linePath);
                            idx++;
                            ...
                            self.appliances_geoJSON.push(linePath.toGeoJSON());
                        }
                    });
                });
            });
        ...
        processMapItemGroup: function( svgXml, groupName, layerGroup) {
            ...
            try {
                groupEl.selectAll("#"+groupEl.attr("id") + " > g").each(function() {
                    var label = "";
                    ...
                    d3.select(this).selectAll("path").each(function () {
                        ...
                        if (linePath) {
                            ...
                            self.map_items.push({
                                "order": idx,
                                "type": groupName,
                                "label": label ? label : groupName + " " + idx,
                                "latitude": center.lat,
                                "longitude": center.lng,
                                // "bounds": bounds
                            });
                            self.map_items_geoJSON.push(linePath.toGeoJSON());
                        }
                    });
                });

            } catch(x) {
                console.log("Failed to process appliance group: " + groupName, x);
            }
            //this.updateProgress(groupName, idx);
        ...
        prettifyLabel: function( label) {
            ...
            for (var i = 0; i < words.length; i++ ) {
                prettier += words[i][0].toUpperCase() + words[i].substring(1) + ' ';
            }
            return prettier;
        },
        ...
        plantZoneForMapItemCenter: function( center) {
            for (var plant_zone_id in this.plant_zone_rects) {
                if (this.plant_zone_rects[plant_zone_id].getBounds().contains(center)) {
        ...
        zonesForMapItem: function( itemPoly) {
            var alle_zoner = [];
            for (var zone_id in this.zone_rects) {
                if (this.zone_rects[zone_id].getBounds().intersects(itemPoly.getBounds())) {
        ...
        roomIdForZone: function( itemPoly) {
            for (var zone_id in this.roomZones) {
                if (this.roomZones[zone_id].getBounds().contains(itemPoly.getBounds())) {
                    return zone_id;
                }
            }
        },
        ...
        pathToPolyLine: function( svgPath, polylineOptions) {
            ...
            try {
                ...
                var nextPoint = this.segmentToPoint(parsedPath[1], initialPoint, initialPoint);

                if (initialPoint && nextPoint) {
                    linePath = L.polyline([
                            this.map.containerPointToLatLng(initialPoint, this.zoomScale),
                            this.map.containerPointToLatLng(nextPoint, this.zoomScale)
                        ], polylineOptions);

                    var currentPoint = nextPoint;
                    for (var i = 2; i < parsedPath.length; i++) {
                        nextPoint = this.segmentToPoint(parsedPath[i], currentPoint, initialPoint);
                        if (nextPoint) {
                            linePath.addLatLng(this.map.containerPointToLatLng(nextPoint, this.zoomScale));
                        }
        ...
        segmentToPoint: function( segment, currentPoint, initialPoint) {
            ...
            if (command == 'M') { // move-to
                return L.point(parseFloat(segment[1])*this.xConvFactor,parseFloat(segment[2])*this.yConvFactor);
            } else if (command== 'V') { // vertical line
                return L.point(currentPoint.x,parseFloat(segment[1])*this.yConvFactor);
            } else if (command == 'H') { // horizontal line
                return L.point(parseFloat(segment[1])*this.xConvFactor,currentPoint.y);
            } else if (command== 'v') { // relative vertical line
                return L.point(currentPoint.x, currentPoint.y + parseFloat(segment[1])*this.yConvFactor);
            } else if (command == 'h') { // relative horizontal line
                return L.point(currentPoint.x + parseFloat(segment[1])*this.xConvFactor,currentPoint.y);
            } else if (command == 'L') { // absolute line-to
                return L.point(parseFloat(segment[1])*this.xConvFactor,parseFloat(segment[2])*this.yConvFactor);
            } else if (command == 'l') { // relative line-to
                return L.point(currentPoint.x + parseFloat(segment[1])*this.xConvFactor,currentPoint.y + parseFloat(segment[2])*this.yConvFactor);
            } else if (command == 'C') { // cubic bezier
                // TODO: actually draw curves
                return L.point(parseFloat(segment[5])*this.xConvFactor,parseFloat(segment[6])*this.yConvFactor);
            } else if (command == 'c') { // relative cubic bezier
                // TODO: actually draw curves
                return L.point(currentPoint.x + parseFloat(segment[5])*this.xConvFactor,currentPoint.y +parseFloat(segment[6])*this.yConvFactor);
            } else if (command == 'S' || command == 'Q' || command == 'T') { // other bezier
                // TODO: actually draw curves
                return L.point(parseFloat(segment[5])*this.xConvFactor,parseFloat(segment[6])*this.yConvFactor);
            } else if (command == 's' || command == 'q' || command == 't') { // relative other bezier
                // TODO: actually draw curves
                return L.point(currentPoint.x + parseFloat(segment[5])*this.xConvFactor,currentPoint.y +parseFloat(segment[6])*this.yConvFactor);
            } else if (command == 'a' || command == 'A') { // elliptical
                // TODO: actually draw curves
                return L.point(initialPoint.x, initialPoint.y);
            } else if (command == 'z' || command == 'Z') { // close path
                return L.point(initialPoint.x, initialPoint.y);
            } else {
                console.warn('failed to create point for path command => ' + segment);
            }
        }
...
$(document).ready(function() {
    ...
    var bboxWidth = 100, bboxHeight = 50; // meters
    ...
    boundsControl.onAdd = function (map) {
        ...
        L.DomEvent.addListener(barContainer, "click", boundsControl.onClick, this);
        L.DomEvent.addListener(barContainer, "dblclick", boundsControl.onClick, this);
        L.DomEvent.addListener(barContainer, "mousedown", boundsControl.onClick, this);
    ...
    var updateMap = function(facilityId) {
        ...
        var lat = 40.01734, lon = -105.27168;
        var growIcon  =  L.icon({
    ...
    $("input[name='floorplan_image']").on('change', function(evt) {
        var floorplanImage = document.querySelector("input[type=file][name=floorplan_image]");
		...
		if (floorplanImage.files.length > 0) {
            ...
            setTimeout(function() {
                ...
                if (file.type == "image/svg+xml") {
                    d3.xml(objUrl).mimeType("image/svg+xml").get(function(error, xml) {
                        if (error) {
                            throw error;
                        }

                        var importer = new GrowServer.Floorplan.Importer({"map": mymap});
                            ...
                            if (group == 'done') {
                                ...
                                if (group == 'zones') {
                                    $.post("/floorplans/add.json", {
                                        ...
                                        "floor_level": $("input[name='floor_level'").val(),
                                        "geoJSON": JSON.stringify(this.floorplan),
                                        "latitude": JSON.stringify(this.center.lat),
                                        "longitude": JSON.stringify(this.center.lng),
                                        "zones": JSON.stringify(this.zones),
                                        "zones_geoJSON": JSON.stringify(this.zones_geoJSON)
                                    },function(response) {
                                        console.log("saved floorplan!");
                                        console.log(response);
                                        floorplanId = response.floorplan.id;
                                        $("#import-summary").append(
                                            '<div id="import-group-'+group+'" class="import-group">'+group+' => '+ count +'</div>'
                                        );
                                        $("#importingModal .progress").width(((parseInt(Math.random()*3)+1)*progressIncrement)*(progress+=(parseInt(Math.random()*3)+1)) + "%");
                                        importer.moreProgress();
                                    });
                                } else if (group != "floorplan") {
                                    var data = {};
                                    data[group] = JSON.stringify(this[group]);
                                    data[group +"_geoJSON"] = JSON.stringify(this[group+"_geoJSON"]);
                                    $.post("/floorplans/layers/" + group + ".json", data, function() {
                                        $("#import-summary").append(
                                            '<div id="import-group-'+group+'" class="import-group">'+group+' => '+ count +'</div>'
                                        );
                                        $("#importingModal .progress").width(((parseInt(Math.random()*3)+1)*progressIncrement)*(progress++) + "%");
                                        importer.moreProgress();
                                    });
                                } else {
                                    $("#importingModal .progress").width((((parseInt(Math.random()*3))*progressIncrement)*progress) + "%");
                                    importer.moreProgress();
                                }
                    ...
                    $("#import-summary").append("No SVG file type detected, 0 floorplan layers imported.");
                    $("textarea[name='geoJSON']").val(JSON.stringify(boundsPolygon.toGeoJSON()));
                }
